...
## Define function for getting folds
## This is unbalanced for now
## Maybe it's worth adding a way of balancing it
...
rank <- c('genus', 'species', 'strain')
msg <- paste0(
    'Performing cross-validation for ', phys_name, ' at the ', 
    paste0(rank, collapse = ', '), ' level(s).'
)
log_print(msg, blank_after = TRUE)
msg <- paste0(
    'Importing ', length(phys_name), ' dataset for propagation: ',
    paste0(phys_name, collapse = ', '), '.'
)
log_print(msg, blank_after = TRUE)
...
msg <- paste0(
    'Checking if ', phys_name, ' is of type range. If so, it will be converted',
    ' to type multistate-intersection based on thresholds.'
)
log_print(msg, blank_after = TRUE)
...
phys <- map(test_sets, ~ {
    ## I still should have some taxids with NAs at this step
    ## This is for the getDataReady function in taxPPro
    filtered_bp_data |> 
        filter(!NCBI_ID %in% unique(.x))
})
...
output <- foreach(i = seq_along(phys_data_ready)) %dopar% {
    time1 <- Sys.time()

...
## Chunk for getting some hold-outs
## taxids in data.tree structure
data('tree_list')
tree <- as.Node(tree_list)
tree_ids <- unname(tree$Get(function(node) node$name))

## I only want to compare  with taxids that are actually be found in the tree
myDF <- data_ready$aerophilicity
...
data_ready$aerophilicity_prediction_all <- myDF |> 
    filter(!NCBI_ID %in% holdout_taxids)


sum(holdout_all$NCBI_ID %in% data_ready$aerophilicity_prediction_all$NCBI_ID)
unique(holdout_all$Evidence)

## Hold-out - genus
regex_g <- '^g__'
## Taxids at the genus level, can be found in the tree, and have not been predicted through early ASR
...
data_ready$aerophilicity_prediction_gn <- myDF |> 
    filter(!NCBI_ID %in% holdout_gn_taxids)

sum(holdout_gn$NCBI_ID %in% data_ready$aerophilicity_prediction_gn$NCBI_ID)
unique(holdout_gn$Evidence)
...
data_ready$aerophilicity_prediction_sp <- myDF |> 
    filter(!NCBI_ID %in% holdout_sp_taxids)

sum(holdout_sp$NCBI_ID %in% data_ready$aerophilicity_prediction_sp$NCBI_ID)
unique(holdout_sp$Evidence)
...
data_ready$aerophilicity_prediction_st <- myDF |> 
    filter(!NCBI_ID %in% holdout_st_taxids)

sum(holdout_st$NCBI_ID %in% data_ready$aerophilicity_prediction_st$NCBI_ID)
unique(holdout_st$Evidence)

holdout_sets <- list(
    aerophilicity_holdout_all = holdout_all,
    aerophilicity_holdout_gn = holdout_gn,
    aerophilicity_holdout_sp = holdout_sp,
    aerophilicity_holdout_st = holdout_st
)
for (i in seq_along(holdout_sets)) {
    write.table(
        x = holdout_sets[[i]],
        file = paste0(names(holdout_sets)[i], '.csv'),
        quote = TRUE, 
        sep = ',', 
        row.names = FALSE
    )
